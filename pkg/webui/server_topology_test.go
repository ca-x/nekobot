...
func TestHandleDeleteChannelAccountRemovesBindings(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.Storage.DBDir = t.TempDir()
	cfg.Agents.Defaults.Workspace = t.TempDir()
	...
	req := httptest.NewRequest(http.MethodDelete, "/api/channel-accounts/"+accountItem.ID, nil)
	rec := httptest.NewRecorder()
	ctx := e.NewContext(req, rec)
	ctx.SetPath("/api/channel-accounts/:id")
	ctx.SetPathValues(echo.PathValues{{Name: "id", Value: accountItem.ID}})
	if err := s.handleDeleteChannelAccount(ctx); err != nil {
		t.Fatalf("handleDeleteChannelAccount failed: %v", err)
	}
	if rec.Code != http.StatusOK {
		t.Fatalf("expected status %d, got %d: %s", http.StatusOK, rec.Code, rec.Body.String())
	...
	if len(bindings) != 0 {
		t.Fatalf("expected bindings to be removed with channel account delete, got %+v", bindings)
	}

...
func (s *BrowserSession) StartWithOptions(timeout time.Duration, opts BrowserStartOptions) error {
	...
	if endpoint != "" {
		if err := s.connectEndpointWithRetry(endpoint, s.timeout); err == nil {
			switch mode {
...
func (s *BrowserSession) connectEndpointWithRetry(endpoint string, timeout time.Duration) error {
	if timeout <= 0 {
		timeout = s.timeout
	}
	connectEndpointFn := s.connectEndpointFn
	if connectEndpointFn == nil {
		connectEndpointFn = s.connectEndpoint
	}
	deadline := time.Now().Add(timeout)
	var lastErr error
	for {
		lastErr = connectEndpointFn(endpoint, timeout)
		if lastErr == nil {
			return nil
		}
		if time.Now().After(deadline) {
